...
\begin{abstract}
On January 20, 2018, the United States government shut down. Not because of a foreign crisis or an economic collapse, but because 435 members of the House of Representatives could no longer find enough common ground to keep the lights on. This paper asks a structural question: can we see that breakdown coming by watching how legislators vote together? We construct co-voting networks for every Congress from the 100th (1987) through the 118th (2023) and analyze their spectral properties, finding that the algebraic connectivity of the congressional graph has collapsed by over 94\% in three decades. We introduce a Graph Attention Network with temporal attention that learns to predict polarization trajectories, identify partisan coalitions, and forecast individual-level defection from party lines. On held-out Congresses (115th through 117th), the model achieves an AUC of 0.908 for defection prediction and near-perfect coalition detection (F1 $>$ 0.97). A difference-in-differences analysis around the Tea Party wave reveals the single largest structural shock in our dataset: a drop in network connectivity that dwarfs even the post-9/11 rally effect. The results suggest that polarization is not just an ideological phenomenon but a topological one, visible in the geometry of legislative cooperation long before it shows up in roll-call scores.
\end{abstract}
...
Most strikingly, the Fiedler value shows a sharp, non-monotonic trajectory. It rises from the 100th to the 107th Congress (peaking at 0.843 in 2001--2003, coinciding with the post-9/11 rally effect), then collapses to near zero by the 113th. Party distance, by contrast, increases smoothly and monotonically. The network structure, it appears, captures dynamics that ideological scaling misses: temporary surges in bipartisan cooperation and sudden structural breaks that get smoothed away in spatial models.
...
The results merit honest discussion. The Random Forest baseline, with access to DW-NOMINATE ideology scores as input features, outperforms the GAT on the defection task. This is not surprising: DW-NOMINATE is itself derived from roll-call voting patterns and is, in a sense, a summary of exactly the behavior we are trying to predict. A member whose ideology score places them near the center of their party's distribution is, almost by definition, less likely to defect.
...
The model slightly overpredicts connectivity for the 116th Congress and slightly overpredicts for the 117th, but the directional trend is correct: it captures the continued near-zero connectivity of the late 2010s and the slight uptick in the 117th Congress.
...
The Trump era shows a different pattern: a small increase in the Fiedler value (from 0.021 to 0.039) alongside a slight increase in party distance. This is not evidence of depolarization. Rather, it reflects the unusual dynamics of intra-party conflict during the Trump years, particularly Republican members breaking with their own party on issues like trade and immigration. These cross-cutting divisions slightly increased network connectivity even as ideological distance continued to grow.

We are cautious about causal claims. Difference-in-differences requires a parallel trends assumption that is difficult to verify with only 19 time points. What we can say is that the structural break around 2010 is the largest in our dataset by a wide margin, and that it is not simply a continuation of the pre-existing trend.
...
Figure~\ref{fig:embeddings} shows t-SNE projections of the learned node embeddings for three Congresses. The embeddings reveal a progressive separation of the partisan clusters: in the 103rd Congress, the two parties overlap substantially in embedding space; by the 118th, they form completely distinct clusters. The model has learned, from network structure and voting patterns, a representation that makes partisan identity almost perfectly separable.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../paper_figures/fig7_embeddings.pdf}
    \caption{t-SNE projections of learned node embeddings for three Congresses. The progressive separation of partisan clusters mirrors the structural disconnection observed in the spectral analysis. By the 118th Congress, the embedding space has become almost perfectly partitioned.}
    \label{fig:embeddings}
\end{figure}

\section{Limitations}

Three limitations deserve explicit acknowledgment. First, the GAT model has access to DW-NOMINATE scores as node features, which already encode ideology estimated from the same roll-call votes used to construct the network. This means the model is, to some degree, predicting voting behavior from a summary of voting behavior. We include DW-NOMINATE because excluding it would be artificial (it is publicly available and universally used), but readers should understand that the model's performance partly reflects the informativeness of this feature rather than the network structure alone. Future work should evaluate a feature-ablated version that relies solely on structural and behavioral features.

Second, our causal analysis is descriptive rather than truly causal. With 19 time points and no control group (there is only one U.S. Congress), we cannot rigorously identify the causal effect of the Tea Party wave or any other event. The difference-in-differences framework provides a useful heuristic, but it should be interpreted as pattern description, not causal identification.

Third, we focus exclusively on the House of Representatives and on the two major parties. The Senate operates under different rules that affect cooperation patterns, and independent or third-party members (though rare) are excluded from our analysis. Extending this framework to bicameral analysis and to state legislatures is a natural direction.

\section{Conclusion}

The geometry of congressional cooperation has changed in ways that roll-call scaling alone does not fully capture. Between 1987 and 2023, the algebraic connectivity of the House co-voting network collapsed by over 94\%, with the single largest structural shock occurring around the Tea Party wave of 2010. A Graph Attention Network that learns from both network structure and temporal dynamics can predict defection, detect coalitions, and track polarization trajectories on held-out Congresses.

But the most important finding may be the simplest one. Polarization is not just about ideology. Two parties can be ideologically distant and still cooperate on enough issues to govern; the U.S. Congress managed this for most of the 20th century. What has changed is not just the distance between the parties but the structure of connections between them. The bridges that once allowed cross-partisan legislative coalitions to form have been systematically dismantled, and our spectral measures show this dismantling with precision.

If these trends continue, the implications extend beyond political science. A Congress that cannot form cross-partisan majorities cannot respond flexibly to crises, cannot compromise on fiscal policy, and cannot provide the stable governance that democratic legitimacy requires. The network structure we document is not an abstract mathematical curiosity; it is the scaffolding on which legislative capacity rests. And that scaffolding is, by the measures we construct, closer to collapse than at any point in the past four decades.
