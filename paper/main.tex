...
\begin{table}[H]
\centering
\caption{Predicted defector probabilities for selected legislators across held-out Congresses. Pred. Prob. is the model's predicted probability of defection (voting against majority of own party on >50\% of contested rollcalls). ``Missed defectors'' are legislators with documented high defection rates in real world but low model predictions.}
\label{tab:defectors}
\begin{tabular}{lccc}
\toprule
